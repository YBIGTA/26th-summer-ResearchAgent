...
from torch import autocast # <<< 이 임포트 추가

# 이미지 생성을 위한 라이브러리
# pip install diffusers
# from diffusers import StableDiffusionXLPipeline # 기존의 SDXL 파이프라인
from diffusers import StableDiffusionPipeline # <<< 이 라인으로 변경
...
def generate_image_from_prompt(
    ...
    """
    Lambda Labs의 포켓몬 모델을 사용하여 이미지를 생성하고 저장합니다.
    """
    try:
        # 포켓몬 특화 Stable Diffusion 파이프라인 로드
        model_id = "lambdalabs/sd-pokemon-diffusers"
        print(f"Loading Pokemon-specific pipeline from '{model_id}'...")
        pipe = StableDiffusionPipeline.from_pretrained(model_id, torch_dtype=torch.float16)
        pipe = pipe.to("cuda")

        # NSFW(안전성) 검사기 비활성화 (포켓몬 이미지가 오탐될 수 있음)
        # 이 모델은 포켓몬 특성상 안전성 검사기가 오작동할 수 있으므로 비활성화하는 것을 권장합니다.
        def null_safety(images, **kwargs):
            return images, False
        pipe.safety_checker = null_safety
        # NSFW(안전성) 검사기 비활성화 옵션 (포켓몬에서 오탐 잦음)
        if disable_safety:
            def null_safety(images, **kwargs):
                return images, False
            pipe.safety_checker = null_safety
        ...
        # autocast를 사용하여 이미지 생성 (메모리 효율 증가)
        with autocast("cuda"):
            # 단일 이미지를 생성하므로 n_samples는 1로 설정합니다.
            images = pipe(prompt=prompt, guidance_scale=7.5, num_images_per_prompt=1).images
            # num_inference_steps=28, # 필요시 추론 스텝 조정 가능
            images = pipe(
                prompt=prompt,
                guidance_scale=guidance_scale,
                num_images_per_prompt=num_images_per_prompt,
                # num_inference_steps=28,  # 필요시 열어 사용
            ).images
